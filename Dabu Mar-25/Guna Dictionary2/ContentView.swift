...
struct ContentView: View {
    ...
    var filteredEntries: [DictionaryEntry] {
        ...
        if normalizedSearchText.count < 2 {
            return entries  // Return the full list unchanged
        }
        // Prepare search results using prioritization criteria
        let sortedEntries = entries.filter { entry in
            ...
            // Check if the search text is found within either the word or the translation
            ...
            // If word matches are equivalent, prioritize exact translation matches
